...
class TaskRunToRequestMapper:
    ...
    def _make_generate_prompt(self, task_run: TaskGenerateRun) -> str:
        ...
        return f"""
            ...
            – Каждая строка должна быть уникальной, нельзя чтобы офферы повторялись  
        """
...
Затем подбери идеальный фон для карточки товара. Он должен подчёркивать продукт, не отвлекать внимание, быть визуально привлекательным и соответствовать товарной категории. Приводи фон кратко и конкретно, 1–2 предложения максимум. Учитывай стиль маркетплейсов:  – техника — нейтральный, серый или графитовый;  – косметика — светлый, мягкий градиент;  – кухня — lifestyle, уютная атмосфера;  – спорт — динамичный, контрастный;  – мебель — интерьерный, с акцентом на фактуру.
Затем предложи 3 чётких оффера (преимущества товара) — каждый из них не более трёх слов, конкретных и сильных. Они должны подчёркивать главные достоинства и побуждать к покупке.
Затем опиши оптимальный стиль иконок, подходящий для визуализации преимуществ. Укажи, какими они должны быть (например: «3D с мягкими тенями», «минимализм с текстурами», «объёмные в стиле flat» и т.д.). Не более пяти слов.
