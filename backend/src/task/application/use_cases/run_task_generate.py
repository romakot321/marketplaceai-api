...
class RunTaskGenerateUseCase:
    ...
    async def execute(self, task_id: UUID, dto: TaskGenerateCreateDTO, file: BytesIO | None = None) -> None:
        """Run it in background"""
        command = TaskGenerateRun(**dto.model_dump(), file=file)
    ...
    def _make_file_url(self, task_id: UUID) -> str:
        return f'https://{settings.DOMAIN}/api/task/{task_id}/result'
    ...
    async def _store_result(self, task_id: UUID, result: TaskResultDTO) -> Task:
        if isinstance(result.result, BytesIO):
            self.file_storage.write(str(task_id), result.result)
            result.result = self._make_file_url(task_id)
    ...
    async def _wait_for_result(self, external_task_id: str) -> tuple[TaskResultDTO | None, None | str]:
        for _ in range(self.TIMEOUT_SECONDS):
            ...
            elif result_domain.status is TaskStatus.failed:
                logger.warning(result_domain.error)
                return None, result_domain.error

        return None, "Timeout"

    async def _run(self, command: TaskGenerateRun) -> tuple[TaskResultDTO | None, None | str]:
        try:
            result = await asyncio.wait_for(self.runner.start_generate(command), timeout=self.TIMEOUT_SECONDS)
