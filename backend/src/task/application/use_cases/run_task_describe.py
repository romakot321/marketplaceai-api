...
class RunTaskDescribeUseCase:
    ...
    async def execute(self, task_id: UUID, dto: TaskDescribeCreateDTO, file: BytesIO) -> None:
        """Run it in background"""
        command = TaskDescribeRun(**dto.model_dump(), file=file)
    ...
    async def _run(self, command: TaskDescribeRun) -> tuple[TaskResultDTO | None, None | str]:
        try:
            result = await asyncio.wait_for(self.runner.start_describe(command), timeout=self.TIMEOUT_SECONDS)
